...
public class DroneCollisionAvoidance : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        rb.AddForce(_direction * host.moveSpeed, ForceMode.VelocityChange);
    }
    private void OnCollisionStay(Collision collision)
    {
        Vector3 _direction = -(collision.transform.position - transform.position).normalized;
        transform.Translate(_direction * Time.deltaTime * host.moveSpeed, Space.World);
    }
    */
    ...
    void CollisionPrevention()
    {
        if (collisionPrevention.isColliding == true)
        {
            ...
            Vector3 _direction = -(collisionPrevention.collision.transform.position - transform.position).normalized;
